...
```Scala
package TallerSemana10Individual
...
object SumaReactivaDeNumeros extends App {
  ...
  //LISTA VACIA
  //val calificaciones: Var[List[Int]] = Var(Nil)

  val sumaTotal = Signal {calificaciones.value.sum}

  val observer = sumaTotal.changed.observe { s =>println(s"Suma actual = $s")}

  calificaciones.set(calificaciones.now :+ 10)
  calificaciones.set(calificaciones.now :+ 20)
}
```
<img width="445" height="98" alt="image" src="https://github.com/user-attachments/assets/be480656-292a-45ff-a273-1449cf01900c" />
...
object EnergiaCineticaReactiva extends App {
  ...
  val masa: Var[Double] = Var(1000)
  val velocidad: Var[Double] = Var(30)

  val energiaCinetica = Signal {0.5 * masa.value * math.pow(velocidad.value, 2)}

  val observer = energiaCinetica.observe { ek => println(s"Energía Cinética: $ek")
    if (ek > 1000000) {
...
```
<img width="476" height="172" alt="image" src="https://github.com/user-attachments/assets/a097c79a-71c9-4706-b8bf-a509442ec206" />
